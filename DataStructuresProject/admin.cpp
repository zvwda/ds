...
void Admin::add_student(string name, int id, string password, int max_hours, int academic_year) {
	...
	student.max_hours_allowed = max_hours;
...
void Admin::View_List_of_all_students_in_a_specific_course(Course c)
{
	...
	for (auto it = GlobalDataa::students.begin(); it != GlobalDataa::students.end(); ++it) {
...
{
	if (input_name == this->name && input_password == this->password) {
